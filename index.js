...
function processSum(numberList,callback) {
  return callback(
    numberList.reduce((total,currentValue) => total += currentValue, 0
    )
  )
}
...
function getFullNames(runners) {
  ...
  // runnersName.push(`${runners.last_name}, ${runners.first_name}`)
  // }
  //  return runnersName;
  let runnersNames = [];
  runners.forEach(i => runnersNames.push(`${runner.last_name}, ${runner.first_name}`));
...
function firstNamesAllCaps(runners) {
  return runners.map(runnerAllcap => runnerA.first_name.toUpperCase());
}
